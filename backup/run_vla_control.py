...
from src.ik_solver import solve_IK
from core.diagnostics_n_logging import make_control_logger, init_hud, update_hud


CAMERA_FOR_VLA = "panda_eye_in_hand"
# 'panda_eye_in_hand'  # use a named camera in XML, e.g. "camera_top"; None uses free cam
...
def apply_action_to_pose(env, action, body_name="panda_eef"):
    ...
    pos_scale = 0.05  # meters per step, tune
    rot_scale = 0.2   # radians per step, tune
...
def main():
    # ---- MuJoCo env ----
    xml_path = (
        "/home/es_admin/vla-franka/"
        "Simple-MuJoCo-PickNPlace/asset/panda/franka_panda_w_objs.xml"
    )
    ...
    # if only renderer.update_scene(env.data) it shows the whole environment
    ...
    # ---- VLA setup ----
    instruction = "go over the blue cube"
    ...
    while env.tick < max_tick:
        ...
        if env.tick % steps_per_policy == 0:
            ...
            # action = np.array(
            #     [0.20, 0.20, -0.10, 0.0, 0.0, 0.0, 1.0]
            # )  # dummy action for debugging IK solver and PID

            # 4) convert action -> EE target pose + gripper
            p_trgt, R_trgt, gripper_q = apply_action_to_pose(
                env, action, body_name="panda_eef"
            )
        ...
        # print(f"[{env.tick}] arm_err={err_arm:.4f}, "
        #       f"grip_err={err_grip:.4f}, "
        #       f"total={np.linalg.norm(desired_q - q_curr_all):.4f}")
